...
<script>
	...
	export default{
		...
		methods:{
			...
			go_item(data){
				let url="/pages/report/hr/RecruitmentReport/RecruitmentReport_item?adminId="+this.search.adminId+'&month='+this.$moment(this.month).format('YYYY-MM')+'&createDate='+data.createDate
				let string='';
				if(data=='searchPopup'){		
					...
					url="/pages/report/hr/RecruitmentReport/RecruitmentReport_item?adminId="+this.search.adminId+string
				}
				uni.navigateTo({
					url:url,
