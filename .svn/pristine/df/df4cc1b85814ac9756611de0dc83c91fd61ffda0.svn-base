		...
		<view class="content">
			...
			<view class="main">
				...
				<view style="display: flex;">
					<label for="" style="padding-right: 8px;">记住密码:</label>
					 <u-switch v-model="checked" asyncChange @change="checked=!checked" ></u-switch>
				</view>
			</view>
			<wButton 
				class="wbutton"
...
<script>
	...
	export default {
		...
		created() {
			try{
				this.form=uni.getStorageSync('account')?JSON.parse(uni.getStorageSync('account')):{}
			}catch(err){
				console.log(err)
			}
		...
		methods: {
		    startLogin(e){
				...
				this.$Api.login.login(user).then(res=>{	
					this.isRotate=false;
					if(!res || res.data.code != 200 ){
						uni.showToast({icon: 'none',position: 'top',title: res.data.message});				
					}
					else if(res.data.code == "200" || res.data.message == "已登录"){
						user.truename = res.data.data.truename;
						this.form.token   = res.data.data.accessToken;
						this.form.truename= res.data.data.truename;
						user.accessToken=res.data.data.accessToken;
						this.checked==true?uni.setStorageSync('account', JSON.stringify(user)):uni.removeStorageSync('account');
			...
			//获取菜单
			get_menu(){
				this.$Api.index.GetSystemMenu().then(res=>{
					console.log(res)
					console.log(123)
					if(res.data.code==200){
						uni.setStorageSync('menu',JSON.stringify(res.data.data));
						uni.reLaunch({
						    url: res.data.data.menus[0].path,
						});
					}else{
						uni.showToast({icon: 'none',position: 'top',title: res.data.message});
					}
