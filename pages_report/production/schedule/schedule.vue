	...
	export default{
		data(){
			return{
				search:'',
				data:[],
				column:[{label:'订单号',prop: 'order',width:'110px'},
				{label:'名称',prop: 'invName'},
				{label:'计划数量',prop: 'quantity',sum:true,width:'80px'},
				{label:'入库数量',prop: 'orderQualifyQty',sum:true,width:'80px'},
				],
				footer:[],
				loading:false,
			}
		...
		methods:{
			GetList(date=[this.$moment().format('YYYY-MM-DD'),this.$moment().format('YYYY-MM-DD')]){
				...
				let obj={
					'pagesize' :10000000,
					'page' :1,
					'search[packDate][0]' :date[0],
					'search[packDate][1]' :date[1] ?? date[0] ,
				}
				this.$Api.report.production.schedule.GetList(obj).then((res)=>{
