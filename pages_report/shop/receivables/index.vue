		...
		<view>
		     <u-popup :show="popupShow" mode="center" :round="10"   @close="popupClose">
				   <view class="timeWarpper">
						...
						<u-form-item label="类型" 	borderBottom ref="item1" labelWidth='80' @click='pickerShow=true'>
							<u--input
													v-model="form.label"
													disabled
													disabledColor="#ffffff"
	...
	export default {
		...
		methods: {
			GetList(data){
				...
				this.$Api.report.shop.GetList(obj).then((res)=>{
              ...
              dataFormatting(){
				  this.footer=[];
				  let footer={imonth:'总合计',dzzj:0,wjje:0,bcje:0};
				  if(Array.isArray(this.data)){
					  this.data.forEach((item,index)=>{
					  					  footer.dzzj  += parseFloat(item.dzzj);
					  					  footer.wjje  += parseFloat(item.wjje);
					  					  footer.bcje  += parseFloat(item.bcje);
					  })
				  }
